...
sun8i_r_timer_get_timeout(struct device *dev, uint32_t *timeout)
{
	...
	if (mmio_read_32(dev->regs + CONTROL_REG(index)) & BIT(0))
		*timeout = mmio_read_32(dev->regs + VALUE_REG(index));
	else
		*timeout = 0;
...
sun8i_r_timer_set_timeout(struct device *dev, uint32_t timeout)
{
	...
	running = mmio_read_32(dev->regs + CONTROL_REG(index)) & BIT(0);
	/* Pause the timer if it is already running. */
	if (running) {
		mmio_clr_32(dev->regs + CONTROL_REG(index), BIT(0));
...
sun8i_r_timer_irq(void *param)
{
	...
	mmio_write_32(dev->regs + IRQ_STATUS_REG, BIT(index));
